...
const S: Record<string, Tri> = {
  ...
  // ── v3.1: giriş ekranı + kullanım şeritleri + menü aksiyonları ──
  "login.eb": { tr: "Kısıtlı alan", en: "Restricted area", es: "Área restringida" },
  "login.sub": { tr: "Erişim şifresini gir — koç/müdür ekranıdır.", en: "Enter the access password — this is the coach/manager surface.", es: "Introduce la contraseña — pantalla de coach/gerente." },
  "login.ph": { tr: "Şifre", en: "Password", es: "Contraseña" },
  "login.btn": { tr: "Giriş", en: "Enter", es: "Entrar" },
  "login.err": { tr: "Şifre doğrulanamadı — tekrar dene.", en: "Password not verified — try again.", es: "Contraseña no verificada — inténtalo de nuevo." },
  "how.bugun": { tr: "Onayla / Planla doğrudan burada · her satır ilgili bölüme götürür", en: "Approve / Schedule right here · every row deep-links to its section", es: "Aprueba / Planifica aquí mismo · cada fila enlaza a su sección" },
  "how.ekip": { tr: "Karta tıkla → hızlı önizleme · iş tipi ve evreye göre filtrele", en: "Tap a card → quick peek · filter by job type and stage", es: "Toca una tarjeta → vista rápida · filtra por puesto y etapa" },
  "how.profil": { tr: "Soldan kişi seç ya da ara · keşfi Planla · aptitude önerisini Onayla", en: "Pick or search a person on the left · Schedule discovery · Approve the aptitude suggestion", es: "Elige o busca a la izquierda · Planifica el descubrimiento · Aprueba la sugerencia" },
  "how.defter": { tr: "Aday seç · konuyu işaretle (tek not) · Yetkinlik hücresine tıkla → seviye", en: "Pick a candidate · mark a topic (single note) · tap a competency cell → level", es: "Elige candidato · marca el tema (una nota) · toca la celda → nivel" },
  "how.hafiza": { tr: "Kişi seç · nota tıkla → kâğıt form · gözlemi yaz → Çıkar → Onayla", en: "Pick a person · tap a note → paper form · write → Extract → Confirm", es: "Elige persona · toca una nota → ficha · escribe → Extraer → Confirmar" },
  "how.usta": { tr: "Eşleşmeyi Onayla / Düzenle · Yeniden optimize → model dünden öğrenir", en: "Confirm / Edit the pairing · Re-optimize → the model learns from yesterday", es: "Confirma / Edita el emparejamiento · Reoptimiza → el modelo aprende" },
  "how.yerlestirme": { tr: "Önerileri tek tek Uygula → kişiler chart'a akar, cep yumuşar · Sıfırla geri alır", en: "Apply suggestions one by one → people flow into the chart, the pocket eases · Reset undoes", es: "Aplica una a una → la gente fluye al cuadro, el hueco se relaja · Restablecer deshace" },
  "how.saha": { tr: "Zone'a tıkla → ne istediğini ve kimin uyduğunu gör · müsait eli Ekle", en: "Tap a zone → see what it demands and who fits · Add the available hand", es: "Toca una zona → mira qué exige y quién encaja · Añade la mano libre" },
  "m.actions": { tr: "Hızlı aksiyonlar", en: "Quick actions", es: "Acciones rápidas" },
  "m.pendingApt": { tr: "Onay kuyruğu", en: "Approval queue", es: "Cola de aprobación" },
  "m.discPlan": { tr: "Keşif planla", en: "Schedule discovery", es: "Planificar descubrimiento" },
